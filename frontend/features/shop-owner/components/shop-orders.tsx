...
export function ShopOrders() {
  ...
  const { data, isLoading, isError, refetch } = useShopOrders({
    page,
    limit: 10,
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-3xl border bg-card p-4">
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-[1fr_12rem_12rem_10rem_10rem_auto]">
          ...
          />
          <Button
            render={<Link href={`/${locale}/shop-owner/orders/new`} />}
            className="rounded-full"
          >
      ...
      {isLoading ? (
        ...
        <Card className="rounded-3xl">
          <CardContent className="flex min-h-64 flex-col items-center justify-center gap-3">
            <ClipboardList className="size-9 text-destructive" />
            <p>{t("orders.error")}</p>
        ...
        <>
          <div className="grid gap-3">
            {data.orders.map((order) => (
              <Link
                key={order.id}
                href={`/${locale}/shop-owner/orders/${order.id}`}
                className="grid gap-3 rounded-3xl border bg-card p-5 transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md sm:grid-cols-[1fr_auto_auto] sm:items-center"
              >
                <div>
                ...
                <div className="sm:text-end">
                  <p className="font-semibold">{money(order.customerTotal)}</p>
          ...
          <div className="flex items-center justify-between rounded-full border bg-card px-4 py-2">
            ...
            <div className="flex gap-2">
              ...
              <span className="flex min-w-16 items-center justify-center text-sm">
