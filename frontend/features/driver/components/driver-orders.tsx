...
const filters: Array<OrderStatus | "ACTIVE"> = [
  "ACTIVE",
  "ACCEPTED",
  "PICKED_UP",
  "OUT_FOR_DELIVERY",
...
export function DriverOrders() {
  ...
  const { data, isLoading, isError, refetch } = useDriverOrders({
    page,
    limit: 8,
    status: queryStatus,
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        ...
        >
          <SelectTrigger className="h-10 w-full rounded-full bg-card sm:w-56">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {filters.map((f) => (
              <SelectItem value={f} key={f}>
                {f === "ACTIVE" ? t("orders.active") : t(`statuses.${f}`)}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {visible.map((order) => (
            <Link
              key={order.id}
              href={`/${locale}/driver/orders/${order.id}`}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-2 font-semibold">{order.customerName}</h2>
                </div>
                <StatusBadge
                  status={order.status}
              ...
              <div className="mt-5 space-y-2 border-t pt-4 text-sm">
                <p className="flex items-center gap-2">
                  ...
                  <span className="truncate">{order.shopOwner.user.name}</span>
                </p>
