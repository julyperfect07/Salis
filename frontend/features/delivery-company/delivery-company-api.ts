...
export async function getCompanyDashboard() {
  return (
    await api.get<CompanyDashboardResponse>(
      "/orders/delivery-company/dashboard",
    )
  ).data;
}
...
export async function getCompanyOrders(query: CompanyOrdersQuery) {
  return (
    await api.get<CompanyOrdersResponse>("/orders/assigned", { params: query })
  ).data;
}
...
export async function assignDriver(id: string, driverId: string) {
  return (
    await api.patch<AdminOrderResponse>(`/orders/${id}/assign-driver`, {
      driverId,
    })
  ).data;
}
...
export async function confirmPayment(id: string) {
  return (await api.patch<AdminOrderResponse>(`/orders/${id}/confirm-payment`))
    .data;
}

export async function getDrivers(page = 1, limit = 9, search?: string) {
  return (
    await api.get<DriversResponse>("/users/drivers", {
