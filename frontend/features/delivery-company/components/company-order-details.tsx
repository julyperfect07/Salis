...
export function CompanyOrderDetails({ orderId }: { orderId: string }) {
  ...
  const payment = useConfirmPayment();
  ...
  const activeDrivers =
    drivers?.drivers.filter((d) => d.user.isActive !== false) ?? [];
  const act = (mutation: typeof accept, success: string) =>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Button
          ...
          render={<Link href={`/${locale}/delivery-company/orders`} />}
        >
          ...
          {order.status === "PENDING" && (
            <>
              ...
              <Button
                ...
                onClick={() => act(accept, t("details.accepted"))}
              >
                {t("details.accept")}
              </Button>
            </>
          ...
          {order.status === "DELIVERED" &&
            order.paymentStatus === "COLLECTED" && (
              <Button
                ...
                onClick={() => act(payment, t("details.paymentConfirmed"))}
              >
                {t("details.confirmPayment")}
              </Button>
            )}
        </div>
      </div>
      <Card className="overflow-hidden rounded-3xl">
        ...
        <CardContent className="grid gap-5 lg:grid-cols-[1fr_.7fr]">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Info
              icon={Building2}
              label={t("details.shop")}
              value={order.shopOwner.user.name}
            />
            <Info
              icon={Truck}
              label={t("details.driver")}
              value={order.driver?.user.name ?? t("orders.unassigned")}
            />
          </div>
          <div className="rounded-2xl bg-muted/50 p-5">
            <p className="text-sm font-medium">{t("details.financials")}</p>
            <Line
            ...
            />
            <Line
              label={t("details.customerTotal")}
              value={money(order.customerTotal)}
              strong
            />
          </div>
        </CardContent>
      ...
      {order.status === "ACCEPTED" && !order.driver && (
        <Card className="rounded-3xl">
          ...
          <CardContent>
            <Select<string>
              disabled={assign.isPending || activeDrivers.length === 0}
              onValueChange={(driverId) => {
                if (driverId)
                  assign.mutate(
                    ...
                    {
                      onSuccess: () => {
                        toast.success(t("details.assigned"));
                        refetch();
                      },
                      onError: () => toast.error(t("details.actionError")),
                    },
            ...
            >
              <SelectTrigger className="h-11 w-full rounded-xl sm:max-w-sm">
                <SelectValue
                  placeholder={
                    activeDrivers.length
                      ? t("details.selectDriver")
                      : t("details.noDrivers")
                  }
                />
              </SelectTrigger>
              <SelectContent>
                {activeDrivers.map((d) => (
                  <SelectItem value={d.user.id} key={d.user.id}>
      ...
      {order.rejectionReason && (
        <Card className="rounded-3xl border-rose-200 bg-rose-50/60 dark:border-rose-900 dark:bg-rose-950/20">
          <CardContent className="p-5">
            ...
            <p className="mt-2 text-sm">{order.rejectionReason}</p>
          </CardContent>
        </Card>
      )}
      <Card className="rounded-3xl">
      ...
      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent className="rounded-3xl sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("details.rejectTitle")}</DialogTitle>
            <DialogDescription>{t("details.rejectHint")}</DialogDescription>
          </DialogHeader>
          <textarea
            value={reason}
            ...
            className="w-full resize-none rounded-xl border border-input bg-transparent px-3 py-2 text-sm outline-none transition focus:border-ring focus:ring-3 focus:ring-ring/30"
          />
          <DialogFooter>
            <Button
              variant="outline"
...
function Line({
  ...
  return (
    <div
      className={`flex justify-between gap-3 py-2 text-sm ${strong ? "mt-2 border-t pt-4 font-semibold" : "text-muted-foreground"}`}
    >
      <span>{label}</span>
