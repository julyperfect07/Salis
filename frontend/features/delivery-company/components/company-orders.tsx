...
export function CompanyOrders() {
  ...
  const { data, isLoading, isError, refetch } = useCompanyOrders({
    page,
    limit: 9,
      ...
      {isLoading ? (
        ...
        <Card className="rounded-3xl">
          <CardContent className="flex min-h-72 flex-col items-center justify-center">
            ...
            <p className="mt-4 font-medium">{t("orders.empty")}</p>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {data.orders.map((order) => (
            <Link
              key={order.id}
              href={`/${locale}/delivery-company/orders/${order.id}`}
              className="group rounded-3xl border bg-card p-5 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md"
            >
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="mt-1 truncate font-medium">
                    {order.driver?.user.name ?? t("orders.unassigned")}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">
                ...
                <div>
                  ...
                  <p className="mt-1 font-medium">{date(order.createdAt)}</p>
                </div>
              </div>
            </Link>
          ))}
